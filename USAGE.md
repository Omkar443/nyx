...
This section is for people who have **never used the bundle before, never used NYX AI Code, or never done bug hunting**. If you're already comfortable with any of those, skim to Section 1.

### What is this bundle, in plain English?

It's a collection of 82 markdown files (called **skills**) that turn NYX AI Code into a methodical bug-hunting assistant.

Without the bundle, asking NYX AI *"is this XSS?"* gets you a generic answer. With the bundle installed, the same question loads the `hunt-xss` skill — which contains specific detection patterns from 681+ disclosed reports, the exact payloads that have worked, and a validation gate that prevents you from filing a false-positive bug report.

You don't "learn" the bundle. You install it once, then describe what you're testing in plain English, and the relevant skill auto-loads. You read it together with NYX AI and follow the steps.
...
2. **NYX AI Code installed** (from https://claude.ai/download) — this is the CLI app, not NYX AI.ai in your browser.
3. **A NYX AI paid plan** (Pro/Team/Max) or an Anthropic API key with credit. Free NYX AI.ai doesn't include NYX AI Code.
...
# Open NYX AI Code in this folder
claude
```

NYX AI Code opens. You'll see a prompt waiting for you to type. Copy-paste this:
...
**What happens next:**
- NYX AI reads your `CLAUDE.md` (the engagement context file `hunt` created)
- NYX AI triggers `bb-methodology` (the 6-phase workflow) and walks you through Phase 1 (Scope)
- NYX AI asks: *"Is this practice / training mode? (No real submissions, just learning.)"* — say **yes**
- NYX AI triggers `web2-recon` or `offensive-osint` and gives you concrete commands to run

**You follow along.** Each time NYX AI gives you a command, paste it in another terminal tab and run it. Tell NYX AI what came back. NYX AI will spot vulnerable patterns and trigger the matching `hunt-*` skill.

For example, when you find Juice Shop's `/api/users` endpoint with an `id` parameter, NYX AI loads `hunt-idor` and walks you through testing for Insecure Direct Object Reference.
...
3. Read the program's scope page carefully — paste it into NYX AI and ask it to parse with `bb-methodology`
...
**How auto-triggering works**: just describe what you're testing — e.g., *"I see a `?url=` parameter on this endpoint"* — and NYX AI loads only `hunt-ssrf`. You don't invoke them by name. The skill matcher looks at your prose and triggers based on the description field.
...
| Tool | Purpose | Setup |
|---|---|---|
| **Burp MCP** | NYX AI reads/replays HTTP traffic directly from Burp's proxy history — eliminates manual paste-curl-into-chat | Burp Suite + MCP Server extension (port 9876) → `claude mcp add burp -s user -- java -jar ~/.BurpSuite/mcp-proxy/mcp-proxy-all.jar` |
...
| **Anthropic API (separate from NYX AI Max)** | Powers `public-skills-builder` for periodic skill regeneration | `console.anthropic.com/billing` → API key → `export ANTHROPIC_API_KEY=...` |
...
**Important — NYX AI Max ≠ API.** NYX AI Max gives you NYX AI Code + chat. The Anthropic API is pay-per-token, billed separately. You need both keys if you want to run skill-generation tools.
...
This creates `CLAUDE.md`, `scope.md`, `findings/`, `evidence/`, `submissions.txt`, `notes.md`, `.gitignore`. Open NYX AI Code from this directory:
...
NYX AI triggers `bb-methodology` and walks through the program rules, populating in-scope assets, OOS, focus areas, bounty bands, and engagement rules.

### Step 3 — Recon

> *"Run a recon pass on `*.acme.com`. I want subdomains, exposed APIs, S3 buckets, and any leaked secrets in JS bundles."*

NYX AI triggers `offensive-osint` and `web2-recon`. If Burp MCP is connected, NYX AI can replay requests from your browser session directly.

### Step 4 — Hunt a specific class

> *"I see `/api/users/{id}/orders` in the JS bundle. Going to test for IDOR with two test accounts."*

NYX AI triggers `hunt-idor`. It walks through detection patterns, suggests payloads (HTTP method swap, array wrap, GraphQL node() resolver), and reminds you to verify with two accounts.
...
Or describe the finding to NYX AI. The `triage-validation` skill runs the 7-Question Gate:
...
Before any screenshot, tell NYX AI:

> *"I'm about to capture a PoC screenshot of the IDOR. What do I need to redact?"*

NYX AI triggers `evidence-hygiene`. You get the cookie redaction protocol, PII black-bar rules, and the screenshot capture order. If you're using Burp Repeater, NYX AI reminds you to drag the divider down to hide the cookie panel.
...
NYX AI triggers `report-writing` (for the body template) and `bugcrowd-reporting` (for VRT mapping, severity request, OOS rebuttals if relevant). The output is a copy-paste-ready report.
