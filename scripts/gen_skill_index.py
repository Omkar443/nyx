#!/usr/bin/env python3
"""
gen_skill_index.py — build nyx/data/skill_index.json from the repo.

The `nyx` CLI needs two things at runtime: each skill's `description:` frontmatter
(for `nyx classify`'s matcher) and the list of disclosed-report filenames (shown as
pointers). When `nyx` is pip-installed without the full repo, it falls back to this
...
OUT = REPO / "nyx" / "data" / "skill_index.json"
