# =====================================================================
# install.ps1 — Install NYX Security Intelligence Engine bundle (Windows / PowerShell)
#
# Native Windows port of install.sh. No WSL, no bash required.
# Requires: Windows PowerShell 5.1+ or PowerShell 7+.
#
# DEFAULT (no flags): installs into ~\.claude\ for NYX AI Code —
...
# the plugin marketplace, and the /hunt engine are NYX AI-Code-specific and do
...
#   -All           DETECT installed harnesses and install to each: NYX AI always, ~\.agents
...
#                  strip non-standard keys (sources/report_count) from the NON-NYX AI
...
$BundleName   = 'NYX Security Intelligence Engine'
...
if ($DETECT) {
    ...
    if ($HAS_CLAUDE)   { Write-Host "  + NYX AI Code   -> ~\.claude\skills" }
    ...
    if (-not ($HAS_OPENCODE -or $HAS_CODEX -or $HAS_HERMES)) {
        Write-Host "  (only NYX AI Code detected -- installing there. Force others with -Agents / -Hermes.)"
    }
    Write-Host ''
...
# --- NYX AI Code (always): skills + commands + hunt.ps1 ----------------
Write-Host "Installing NYX Security Intelligence Engine bundle from $RepoDir"
...
if (Test-Path -LiteralPath $CommandsSrc) {
    ...
    Write-Host "Commands ->  $cmdDest   (NYX AI Code only)"
...
if ($NO_PROFILE) {
    ...
    if ($needLine) {
        $new = "# Bug-bounty engagement scaffolding (NYX Security Intelligence Engine)`r`n$dotLine`r`n"
        if (Test-Path -LiteralPath $prof) {
...
"@
        $truncFile = Join-Path $env:TEMP 'nyx_trunc.py'
...
if ($DO_MCP) {
    ...
    if ($mcpTargets.Count -eq 0) {
        Write-Host "  ! -BurpMcp found no non-NYX AI harness (none detected, no -Agents/-Hermes). Skipping."
    } else {
        $py = Get-Python
        if ($py) {
...
Write-Host "============================================"
Write-Host ''
Write-Host "NYX AI Code:   $(Join-Path $HOME '.claude\skills')  (+ commands, hunt.ps1)"
