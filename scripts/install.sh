#!/usr/bin/env bash
# =====================================================================
# install.sh — Install NYX Security Intelligence Engine bundle (multi-harness)
#
...
BUNDLE_NAME="NYX Security Intelligence Engine"
...
# config dir present) and route to each. NYX AI always; ~/.agents only if Codex is present
...
if [ "$DETECT" = "1" ]; then
  ...
  if [ "$HAS_CLAUDE"   = "1" ]; then echo "  ✓ NYX AI Code   → ~/.claude/skills"; fi
  ...
  if [ "$HAS_OPENCODE" = "0" ] && [ "$HAS_CODEX" = "0" ] && [ "$HAS_HERMES" = "0" ]; then
    echo "  (only NYX AI Code detected — installing there. Force others with --agents / --hermes.)"
  fi
  echo ""
...
echo "Installing NYX Security Intelligence Engine bundle from $REPO_DIR"
if [ "$DO_AGENTS" = "1" ] || [ "$DO_HERMES" = "1" ]; then echo "(multi-harness mode)"; fi
echo ""

# === NYX AI Code (always) — skills + commands + hunt.sh ===
install_skills "$HOME/.claude/skills" "skills"

COMMANDS_DEST="$HOME/.claude/commands"
if [ -d "$REPO_DIR/commands" ]; then
  mkdir -p "$COMMANDS_DEST"
  echo "Commands →  $COMMANDS_DEST   (NYX AI Code only)"
...
if [ "$DO_MCP" = "1" ]; then
  ...
  if [ -z "$MCP_TARGETS" ]; then
    echo "  ⚠ --burp-mcp found no non-NYX AI harness (none detected, no --agents/--hermes). Skipping."
...
echo "============================================"
echo ""
echo "NYX AI Code:   $HOME/.claude/skills  (+ commands, hunt.sh)"
