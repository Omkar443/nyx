...
'@
    ...
    # If `nyx recon <target>` produced a manifest, seed scope + notes from it.
...
with open(d + "/scope.md", "a", encoding="utf-8") as f:
    f.write("\n## Recon-seeded live hosts (from nyx manifest — VERIFY in-scope before testing)\n\n")
    for h in hosts:
        f.write(f"- {h}\n")
...
'@
    ...
    Write-Host "  CLAUDE.md           - NYX AI Code engagement context"
