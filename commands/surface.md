---
name: surface
description: Show ranked attack surface for a target from its recon manifest + hunt memory. Deterministic backing is `nyx surface <target>` (reads recon/<target>/manifest.json); LLM layer adds ledger signal. Usage: /surface target.com
---
...
1. Reads the recon manifest at `recon/<target>/manifest.json` (produced by `nyx recon`)
   — the deterministic ranking comes from `nyx surface <target>`
...
> Run `nyx surface <target>` for the raw deterministic ranking.
