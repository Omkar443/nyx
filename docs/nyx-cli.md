...
> **Secondary interface — slash commands are primary.** Inside a NYX Agent conversation, use the slash commands (`/recon`, `/hunt`, `/triage`, `/report`, `/validate`, `/chain`, `/autopilot`, `/scope`, etc.) — they leverage the full skill content and the LLM's judgment.
>
> `nyx` is the **terminal-native deterministic runner** — use it when you're outside NYX Agent, automating in CI/CD, running scheduled recon, or verifying labs reproducibly. Same skills, different execution model.
>
...
## When to use `nyx` vs slash commands

| Use case | Use this |
|---|---|
| Hunting a new target conversationally, applying judgment | **Slash commands** in NYX Agent (`/hunt`, `/triage`, etc.) |
| Building a chain across multiple primitives | **Slash commands** — LLM keeps state across the conversation |
| Scheduled / CI / scripted runs | **`nyx`** — deterministic exit codes, identical output across runs |
| Bulk passive recon (hundreds of subdomains) | **`nyx recon`** — real `subfinder`/`dig`/`curl`, no LLM in the loop |
| Verifying labs / reproducing claims | **`nyx`** — every Phase 2 doc's curls work via `nyx` too |
| Reading skills without NYX Agent installed | **`nyx`** + browsing `skills/` and `docs/disclosed-reports/` |
| Triage gate at PR time / pre-submit linting | **`nyx triage`** — deterministic keyword-match against the 7-Question Gate |
...
> 2. **Burp Suite integration** — `--burp` flag routes everything through Burp's proxy (default `127.0.0.1:8080`). Requests + responses land in Proxy → HTTP history; you can send any of them to Repeater/Intruder/Scanner/Collaborator. Pairs with the **Burp MCP server** (port 9876) for NYX AI-Code-conversational hunting.

## Install

```bash
# Symlink the script to PATH (Linux / macOS)
chmod +x scripts/nyx.py
ln -sf "$(pwd)/scripts/nyx.py" /usr/local/bin/nyx
...
```bash
nyx recon hackerone.com
nyx classify "https://api.target.com/v1/users/42?next=..."
```
...
**A. `--burp` flag — proxy routing.** Routes every HTTP request `nyx` makes through Burp's proxy. Every request + response shows up in Burp Proxy → HTTP history with full reqs/resps captured for replay.

```bash
# Start Burp Suite Pro, ensure Proxy listener is on 127.0.0.1:8080
nyx recon hackerone.com --burp
nyx classify "https://target.com/api/users/42" --burp

# Or use a custom proxy URL (Burp on a different port, mitmproxy, ZAP, etc.)
nyx recon hackerone.com --proxy http://127.0.0.1:8081

# Or set the env var once
export NYX_BURP_PROXY=http://127.0.0.1:8080
nyx recon target.com   # auto-detected
```

What happens after: every host `nyx recon` probes appears in Burp's Target → Site map. Each title-extracted live host is one click from Repeater. You drive the actual attacks from Burp; `nyx` did the bulk discovery + classification.

**B. Burp MCP — conversational hunting via NYX Agent.** If you've installed the Burp MCP server BApp extension (typical port `127.0.0.1:9876`) and registered it with NYX Agent via `claude mcp add burp -s user -- java -jar ~/.BurpSuite/mcp-proxy/mcp-proxy-all.jar`, you can do the hunt loop entirely in a NYX AI conversation:

```
You:  nyx classified /api/users/42 as IDOR-prone. Send it to Burp Repeater
...
This mode is the most ergonomic — the LLM drives Burp via MCP while consulting the skill content. `nyx` and Burp MCP are complementary: `nyx` is fast at bulk classification + structured triage; Burp MCP is fast at deep individual-request analysis.
...
**Operator default:** `--burp` mode if Burp Suite Pro is open; curl-only mode otherwise. Burp MCP mode for engagements where you want maximum LLM-driven workflow inside NYX Agent.

---

## The five subcommands

### `nyx recon <target>` — passive recon + live-host probe
...
### `nyx surface <target>` — ranked attack surface from the recon manifest

```bash
nyx surface hackerone.com
```
...
### `nyx classify <url>` — pattern-match URL → hunt-* skills
...
### `nyx triage <finding.md>` — 7-Question Gate

```bash
nyx triage findings/idor-2026-05-15.md
```

Runs all 7 questions from the `triage-validation` skill against the finding text. Returns:

- **PASS** — all 7 answered with evidence. Eligible for `nyx report`.
...
### `nyx report <finding.md> [--platform h1|bugcrowd|intigriti|immunefi] [--out path]`

```bash
nyx report findings/idor-2026-05-15.md --platform bugcrowd --out submissions/h1-draft.md
```
...
nyx classify "https://api.target.com/v1/users/42?token=abc"
...
nyx triage findings/idor.md
# → PASS — eligible for report drafting

# Day 4 — submit
nyx report findings/idor.md --platform h1 --out drafts/h1-draft.md
...
**Choice not dogma:** operators with no Burp run curl-only; operators with Burp Pro route via `--burp`; operators with Burp + MCP drive everything from a NYX Agent conversation. All three are first-class supported.
