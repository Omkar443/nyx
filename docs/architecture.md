...
The NYX Security Intelligence Engine bundle maps to a 6-phase workflow that supports both bug hunting and external red-team engagements. Each phase has a focused set of skills; skills compose left-to-right as you move through the workflow, but you can jump in at any phase mid-engagement.
...
**How auto-triggering works**: just describe what you're testing — e.g., *"I see a `?url=` parameter on this endpoint"* — and NYX AI loads only `hunt-ssrf`. You don't invoke them by name.
...
| Tool | Purpose |
|---|---|
| **Burp MCP** | NYX AI reads/replays HTTP traffic directly from Burp's proxy history. Eliminates manual paste-curl-into-chat. |
...
1. SCOPE
   ...
   $ cd ~/Targets/acme-bb
   $ claude                    → opens NYX AI Code in this folder
