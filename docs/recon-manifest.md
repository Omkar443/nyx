...
| Producer | Fills |
|---|---|
| `nyx recon <target>` | `target`, `generated_at`, `counts`, `assets`, `ranked_surface` |
...
| Consumer | Uses |
|---|---|
| `nyx surface <target>` | prints the ranked **P1 / P2 / Kill** surface from `ranked_surface` |
...
| `producers` | string[] | e.g. `["nyx-recon/2.1.0"]` |
...
```bash
nyx recon acme.com                 # → recon/acme.com/manifest.json   (producer)
nyx surface acme.com               # ranked P1/P2/Kill                 (consumer)
