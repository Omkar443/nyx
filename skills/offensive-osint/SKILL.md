...
> **v3.0** — Refactored 2026-05-02 from a 4,168-line monolith into a lean SKILL.md (~400 lines) plus 15 modular reference files in `references/`. Detail content loads on demand — NYX Agent reads only the reference files relevant to the current task.
...
Drop these prompts into a fresh NYX Agent session to verify the skill loads correctly.
