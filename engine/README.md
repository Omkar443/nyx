...
```
> **OSINT (subdomain/asset enum) is a *separate* concern** — handled by NYX AI-OSINT, not the
...
| `osint.py` | **separate/optional** — per-target service/tech probe (PD `httpx-toolkit`) reused by recon. Its subdomain-enum path is *not* in the engine flow (that's NYX AI-OSINT). |
