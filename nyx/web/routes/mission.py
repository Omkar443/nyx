...
async def init_mission(
    ...
    ok, data = _parse_res(service.init_engagement(target=req.target, reset=req.reset, force=req.force))
    if not ok:
        raise HTTPException(
    ...
    await emit_event(
        ...
        data={"target": req.target, "action": "init"},
        mission_id=req.target,
    )
...
async def transition_mission_state(
    ...
    await emit_event(
        event_type="mission_completed" if req.new_state == "REPORTING" else "finding_updated",
...
async def get_mission_history(service: EngagementService = Depends(get_engagement_service)) -> Dict[str, Any]:
    ...
    _, data = _parse_res(service.export_engagement())
    return {"success": True, "data": data}
