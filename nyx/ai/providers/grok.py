"""
NYX Grok/xAI Provider Integration
Integrates xAI using the openai Python SDK with bounded timeouts,
...
def _run_daemon_bounded(
    ...
    if t.is_alive():
        raise TimeoutError(f"xAI API connection timed out after {total_timeout_sec} seconds")
...
class GrokProvider(AIProvider):
    ...
    def _get_client(self) -> tuple[Optional[Any], Optional[str]]:
        ...
        if not key:
            return None, "XAI_API_KEY environment variable is not configured in the current process environment"
        ...
        try:
            ...
            return None, f"Failed to initialize xAI client: {_sanitize_error(str(ex))}"

    def get_info(self) -> Dict[str, Any]:
        """Return provider status and configuration info without leaking credentials."""
        client, err = self._get_client()
        is_configured = bool(self.api_key or os.environ.get("XAI_API_KEY"))
    ...
    def test_connection(self, timeout_ms: int = 15000, total_timeout_sec: float = 20.0) -> Dict[str, Any]:
        ...
        logger.info(f"xAI test_connection request started (model: {self.model_name})")
        try:
            text = _run_daemon_bounded(_do_test, total_timeout_sec=total_timeout_sec)
            logger.info("xAI response received successfully")
            return {
                "provider": self.provider_name,
                "model": self.model_name,
                "success": True,
            ...
            logger.error("xAI API connection timed out")
            return {
                "provider": self.provider_name,
                "model": self.model_name,
                "success": False,
            ...
            logger.error(f"xAI API error category: {err_dict['status']}")
            return err_dict

    def generate(self, prompt: str, options: Optional[Dict[str, Any]] = None) -> str:
        client, err = self._get_client()
        if not client:
            raise ValueError(f"Grok provider not initialized: {err}")
        ...
        logger.info(f"xAI generate request started (model: {self.model_name})")
        try:
            ...
            logger.error("xAI API connection timed out")
            raise RuntimeError("Grok API Error: xAI API connection timed out")
        except Exception as ex:
